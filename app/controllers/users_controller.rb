...
class UsersController < ApplicationController
  get '/u/:username', auth: %w[user admin] do
    @user = User.includes(:issues, :comments, :projects).find_by(username: params[:username])

    slim :'users/show'
  end

  get '/settings', auth: %w[user admin] do
    @user = current_user

    slim :'users/edit'
  end

  post '/settings', auth: %w[user admin] do
    @user = current_user
    ...
    if params[:a] && params[:a] == @user.auth_token
      ...
      unless [np, npc, op].all?(&:empty?)
        if @user.authenticate(op) && (np == npc)
          ...
          flash :error, "Either old password or combination of new password and password confirmation is incorrect.\
                         <br> If you don't want to change password, leave all password fields blank"
  ...
  get '/u/:username/issues', auth: %w[user admin] do
  ...
  get '/u/:username/projects', auth: %w[user admin] do
    @user = User.find_by(username: params[:username])
